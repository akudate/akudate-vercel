...
function Technology_tag({ data }: Props) {
  return (
    <div className="w-full h-auto flex flex-row items-center flex-wrap mt-[10px] ">
      ...
      <div
        className={`tag ${
          data.technology_feature.includes("Codeigniter") ? "" : " hidden"
      ...
      <div
        className={`tag ${
          data.technology_feature.includes("Laravel") ? "" : " hidden"
        }  bg-[#FF291A]    text-white  `}
      >
        Laravel
      </div>
      <div
        className={`tag ${
          data.technology_feature.includes("PHP") ? "" : " hidden"
        }  bg-[#8B92BE]    text-white  `}
      >
        PHP
      </div>
    </div>
